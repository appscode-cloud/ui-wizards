...
$ helm repo update
$ helm search repo bytebuilders-ui/kubedbcom-redissentinel-editor --version=v0.4.3
$ helm upgrade -i kubedbcom-redissentinel-editor bytebuilders-ui/kubedbcom-redissentinel-editor -n default --create-namespace --version=v0.4.3
```
...
| resources.monitoringCoreosComPrometheusRule_redis |             | <code>{"apiVersion":"monitoring.coreos.com/v1","kind":"PrometheusRule","metadata":{"name":"redis","namespace":"demo"},"spec":{"groups":[{"name":"redis.database.default.release-name.rules","rules":[{"alert":"RedisqlDown","annotations":{"description":"MySQL instance is down on {{ $labels.pod }}\n  VALUE = {{ $value }}\n  LABELS = {{ $labels }}","summary":"Redis instance down (instance {{ $labels.pod }})"},"expr":"pg_up{job=\"-stats\",namespace=\"default\" == 0","for":"0m","labels":{"alertname":"redis_instance_down","app":"redis","severity":"critical"}}]}]}}</code>                                                             |
...
```bash
$ helm upgrade -i kubedbcom-redissentinel-editor bytebuilders-ui/kubedbcom-redissentinel-editor -n default --create-namespace --version=v0.4.3 --set metadata.resource.group=kubedb.com
```
...
```bash
$ helm upgrade -i kubedbcom-redissentinel-editor bytebuilders-ui/kubedbcom-redissentinel-editor -n default --create-namespace --version=v0.4.3 --values values.yaml
```
