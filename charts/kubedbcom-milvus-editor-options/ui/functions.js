...
const modeDetails = {
  Standalone: {
    description: 'Single node Milvus without high availability.',
    text: 'Standalone',
  },
  Distributed: {
    description: 'Milvus Distributed mode for horizontal scaling across multiple nodes.',
    text: 'Distributed',
  },
}
...
export const useFunc = (model) => {
  ...
  async function initBundle() {
    ...
    if (!getValue(model, `/spec/admin/databases/Milvus/mode/toggle`)) {
      let defMode = getDefault('databases/Milvus/mode') || ''
      if (defMode === '') {
        const arr = getValue(model, '/spec/admin/databases/Milvus/mode/available') || []
  ...
  function fetchOptions(type) {
    ...
    } else if (returnArray.length > 1 && type.endsWith('/versions')) {
      const path = `/spec/admin/${type}/default`
      commit('wizard/model$update', {
        path: path,
        value: returnArray[returnArray.length - 1],
        force: true,
      })
    }
  ...
  function isMetaStorageExternallyManaged() {
    // watchDependency('model#/spec/metaStorage/externallyManaged')
