...
const modeDetails = {
  Standalone: {
    description: 'Single node Qdrant without high availability.',
    text: 'Standalone',
  },
  DataGuard: {
    description: 'Qdrant Data Guard for high availability, data protection, and disaster recovery',
    text: 'DataGuard',
  },
}
...
export const useFunc = (model) => {
  ...
  async function initBundle() {
    ...
    if (!getValue(model, `/spec/admin/databases/Qdrant/mode/toggle`)) {
      let defMode = getDefault('databases/Qdrant/mode') || ''
      if (defMode === '') {
        const arr = getValue(model, '/spec/databases/Qdrant/mode/available') || []
