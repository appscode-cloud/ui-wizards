...
export const useFunc = (model) => {
  ...
  function onScheduleBackupChange() {
    ...
    if (scheduleBackup === 'no') {
      ...
      deleteKubeDbComZooKeeperDbAnnotation(getValue, model, commit)
    } else {
  ...
  function addOrRemoveBinding() {
    ...
    const labels = getValue(model, '/resources/kubedbComZooKeeper/metadata/labels')
    const bindingValues = {
    ...
    if (value) {
      commit('wizard/model$update', {
        path: '/resources/catalogAppscodeComZooKeeperBinding',
        value: bindingValues,
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/catalogAppscodeComZooKeeperBinding')
    }
  }
  ...
  function isBindingAlreadyOn() {
    ...
    const isExposeBinding = !!keys.find((str) => str === 'catalogAppscodeComZooKeeperBinding')
