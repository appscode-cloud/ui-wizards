...
export const useFunc = (model) => {
  ...
  async function getConfigMapKeys(index) {
    ...
    const configMapName = getValue(
      model,
      `/resources/kubedbComZooKeeper/spec/monitor/prometheus/exporter/env/${index}/valueFrom/configMapKeyRef/name`,
    )
  ...
  async function getSecretKeys(index) {
    ...
    const secretName = getValue(
      model,
      `/resources/kubedbComZooKeeper/spec/monitor/prometheus/exporter/env/${index}/valueFrom/secretKeyRef/name`,
    )
  ...
  function onTriggerChange(type) {
    ...
    const commitPath = `/resources/autoscalingKubedbComZooKeeperAutoscaler/spec/${type}/trigger`
  ...
  function onEnvArrayChange() {
    ...
    if (filteredEnv.length)
      commit('wizard/model$update', {
        path: '/resources/kubedbComZooKeeper/spec/monitor/prometheus/exporter/env',
        value: filteredEnv,
        force: true,
      })
  }
  ...
  function initEnvArray() {
    const env = getValue(
      model,
      '/resources/kubedbComZooKeeper/spec/monitor/prometheus/exporter/env',
    )
  ...
  function initMonitoring() {
    const env =
      getValue(model, '/resources/kubedbComZooKeeper/spec/monitor/prometheus/exporter/env') || []
