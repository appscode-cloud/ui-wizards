...
export const useFunc = (model) => {
  ...
  async function getReferSecrets() {
    ...
    const url = `/clusters/${user}/${cluster}/proxy/core/v1/namespaces/${namespace}/secrets`
  ...
  function setMachineToCustom() {
    const path = '/spec/admin/machineProfiles/default'
  ...
  async function initBundle() {
    ...
    db = db.toLowerCase()
    const url = `clusters/${owner}/${cluster}/db-bundle?type=features,common,versions&db-singular=${db}`
    ...
    if (!getValue(model, `/spec/admin/databases/DocumentDB/mode/toggle`)) {
      let defMode = getDefault('databases/DocumentDB/mode') || ''
      if (defMode === '') {
        const arr = getValue(model, '/spec/admin/databases/DocumentDB/mode/available') || []
  ...
  function checkIfFeatureOn(type) {
    ...
    if (type === 'backup') {
      if (backupToolInitialValue === '') {
        ...
        return backupToolInitialValue === 'on'
      }
    } else if (type === 'tls') {
