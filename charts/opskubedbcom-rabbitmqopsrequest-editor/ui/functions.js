...
export const useFunc = (model) => {
  ...
  async function getDbs() {
    ...
    const resp = await axios.get(
      `/clusters/${owner}/${cluster}/proxy/kubedb.com/v1alpha2/namespaces/${namespace}/rabbitmqs`,
      {
        params: { filter: { items: { metadata: { name: null } } } },
      },
    )
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/rabbitmqs/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.RabbitMQ?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const resp = await axios.get(
        `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/rabbitmqversions`,
        {
          params: queryParams,
        },
      )
      ...
      const filteredRabbitMQVersions = sortedVersions.filter((item) => {
        if (limit === '0.0')
          return (
      ...
      setDiscriminatorValue('/filteredVersion', filteredRabbitMQVersions)
      return filteredRabbitMQVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
