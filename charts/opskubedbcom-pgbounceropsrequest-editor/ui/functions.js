...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/pgbouncers/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.PgBouncer?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const filteredPgBouncerVersions = sortedVersions.filter((item) => {
      ...
      setDiscriminatorValue('/filteredVersion', filteredPgBouncerVersions)
      return filteredPgBouncerVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
  ...
  function onReconfigurationTypeChange() {
    ...
    if (reconfigurationType === 'remove') {
      ...
      commit('wizard/model$delete', `/spec/configuration/pgbouncer/configSecret`)
      commit('wizard/model$delete', `/spec/configuration/pgbouncer/applyConfig`)
      commit('wizard/model$delete', `/spec/configuration/pgbouncer/removeCustomConfig`)
    }
  }
