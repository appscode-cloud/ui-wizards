...
function setAddressType({ model, getValue }) {
  const value = getValue(model, '/resources/kubedbComFerretDB/spec/useAddressType')
...
// ************************* Basic Info **********************************************
async function getFerretDBVersions({ axios, storeGet }, group, version, resource) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
...
function setDatabaseMode({ model, getValue }) {
  const replicas = getValue(model, '/resources/kubedbComFerretDB/spec/replicas')
...
const onDatabaseModeChange = ({ discriminator, getValue, commit }) => {
  ...
  commit('wizard/model$update', {
    path: '/resources/kubedbComFerretDB/spec/replicas',
...
async function getIssuerRefsName({ axios, storeGet, getValue, model, watchDependency }) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  watchDependency('model#/resources/kubedbComFerretDB/spec/tls/issuerRef/apiGroup')
  watchDependency('model#/resources/kubedbComFerretDB/spec/tls/issuerRef/kind')
  watchDependency('model#/metadata/release/namespace')
  const apiGroup = getValue(model, '/resources/kubedbComFerretDB/spec/tls/issuerRef/apiGroup')
  const kind = getValue(model, '/resources/kubedbComFerretDB/spec/tls/issuerRef/kind')
  const namespace = getValue(model, '/metadata/release/namespace')
...
function setSSLMode({ model, getValue }) {
  const val = getValue(model, '/resources/kubedbComFerretDB/spec/sslMode')
  return val || 'require'
}
...
function onTlsConfigureChange({ discriminator, getValue, commit }) {
  ...
  if (configureStatus) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComFerretDB/spec/tls',
      value: { issuerRef: {}, certificates: [] },
      force: true,
    })
  } else {
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/tls')
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/sslMode')
  }
}
...
function initPrePopulateDatabase({ getValue, model }) {
  const waitForInitialRestore = getValue(
    model,
    '/resources/kubedbComFerretDB/spec/init/waitForInitialRestore',
  )
  const stashAppscodeComRestoreSession_init = getValue(
    model,
    '/resources/stashAppscodeComRestoreSession_init',
  )
  const script = getValue(model, '/resources/kubedbComFerretDB/spec/init/script')
...
function onPrePopulateDatabaseChange({ commit, getValue, discriminator, model }) {
  ...
  if (prePopulateDatabase === 'no') {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComFerretDB/spec/init/waitForInitialRestore',
      value: false,
    })
    commit('wizard/model$delete', '/resources/stashAppscodeComRestoreSession_init')
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/init/script')
...
function onDataSourceChange({ commit, getValue, discriminator, model }) {
  ...
  if (dataSource === 'script') {
    ...
    if (!valueExists(model, getValue, '/resources/kubedbComFerretDB/spec/init/script'))
      commit('wizard/model$update', {
        path: '/resources/kubedbComFerretDB/spec/init/script',
        value: initScript,
      })
...
function initVolumeType({ getValue, model }) {
  const configMap = getValue(model, '/resources/kubedbComFerretDB/spec/init/script/configMap/name')
  const secret = getValue(model, '/resources/kubedbComFerretDB/spec/init/script/secret/secretName')
...
function onVolumeTypeChange({ commit, getValue, discriminator, model }) {
  ...
  if (sourceVolumeType === 'configMap') {
    ...
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/init/script/secret')

    if (!valueExists(model, getValue, '/resources/kubedbComFerretDB/spec/init/script/configMap')) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComFerretDB/spec/init/script/configMap',
        value: {
          name: '',
        },
      })
    ...
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/init/script/configMap')

    if (!valueExists(model, getValue, '/resources/kubedbComFerretDB/spec/init/script/secret')) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComFerretDB/spec/init/script/secret',
        value: {
          secretName: '',
        },
      })
...
function getBackupConfigsAndAnnotations(getValue, model) {
  ...
  const kubedbComFerretDBAnnotations =
    getValue(model, '/resources/kubedbComFerretDB/metadata/annotations') || {}

  const isBluePrint = Object.keys(kubedbComFerretDBAnnotations).some(
    (k) =>
...
function deleteKubeDbComFerretDBDbAnnotation(getValue, model, commit) {
  const annotations = getValue(model, '/resources/kubedbComFerretDB/metadata/annotations') || {}
  ...
  commit('wizard/model$update', {
    path: '/resources/kubedbComFerretDB/metadata/annotations',
    value: filteredAnnotations,
  })
}

function addKubeDbComFerretDBDbAnnotation(getValue, model, commit, key, value, force) {
  const annotations = getValue(model, '/resources/kubedbComFerretDB/metadata/annotations') || {}
...
function onScheduleBackupChange({ commit, getValue, discriminator, model }) {
  ...
  if (scheduleBackup === 'no') {
    ...
    // delete annotation from kubedbComFerretDB annotation
    deleteKubeDbComFerretDBDbAnnotation(getValue, model, commit)
  } else {
...
function onTaskParametersChange({ getValue, discriminator, model, commit }) {
  ...
  const oldAnnotations = getValue(model, '/resources/kubedbComFerretDB/metadata/annotations') || {}
...
function onNameChange({ commit, model, getValue }) {
  ...
  if (hasSecretConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComFerretDB/spec/configSecret/name',
      value: `${dbName}-config`,
      force: true,
    })
  }
  ...
  if (hasSecretShardConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComFerretDB/spec/shardTopology/shard/configSecret/name',
  ...
  if (hasSecretConfigServerConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComFerretDB/spec/shardTopology/configServer/configSecret/name',
  ...
  if (hasSecretMongosConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComFerretDB/spec/shardTopology/mongos/configSecret/name',
...
function getCreateAuthSecret({ model, getValue }) {
  const authSecret = getValue(model, '/resources/kubedbComFerretDB/spec/authSecret')
...
function disableInitializationSection({ model, getValue, watchDependency }) {
  const initialized = getValue(model, '/resources/kubedbComFerretDB/spec/init/initialized')
  watchDependency('model#/resources/kubedbComFerretDB/spec/init/initialized')
  return !!initialized
}
...
function onCreateAuthSecretChange({ discriminator, getValue, commit }) {
  ...
  if (createAuthSecret) {
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/authSecret')
...
function onConfigurationChangeEdit({ getValue, commit, discriminator, model }) {
  ...
  commit('wizard/model$update', {
    path: '/resources/secret_config/data/ferretdb.ini',
    value: btoa(value),
    force: true,
  })
  const configSecretName = `${getValue(model, '/metadata/release/name')}-config`
...
function setConfigurationForEdit({ model, getValue }) {
  const value = getValue(model, '/resources/secret_config/data/ferretdb.ini')
  return atob(value)
}
...
function onSetCustomConfigChange({ discriminator, getValue, commit }) {
  ...
  if (value === 'no') {
    commit('wizard/model$delete', '/resources/kubedbComFerretDB/spec/configSecret')
    commit('wizard/model$delete', '/resources/secret_config')
  }
}
...
function getOpsRequestUrl({ storeGet, model, getValue, mode }, reqType) {
  ...
  if (mode === 'standalone-step') return pathConstructedForKubedb
    ...
    return `${domain}/${owner}/kubernetes/${cluster}/ops.kubedb.com/v1alpha1/ferretdbopsrequests/create?name=${dbname}&namespace=${namespace}&group=${group}&version=${version}&resource=${resource}&kind=${kind}&page=operations&requestType=VerticalScaling`
}
...
function onRefChange({ discriminator, getValue, commit }) {
  ...
  commit('wizard/model$update', {
    path: '/resources/kubedbComFerretDB/spec/database/databaseRef/name',
    value: ref.name || '',
    force: true,
  })
  commit('wizard/model$update', {
    path: '/resources/kubedbComFerretDB/spec/database/databaseRef/namespace',
    value: ref.namespace || '',
    force: true,
  })
}
...
function clearSpecModel({ commit }, dbtype) {
  if (dbtype === 'standalone') {
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComFerretDBAutoscaler/spec/${autoscaleType}/cluster`,
    )
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComFerretDBAutoscaler/spec/${autoscaleType}/sentinel`,
    )
  } else if (dbtype === 'cluster') {
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComFerretDBAutoscaler/spec/${autoscaleType}/standalone`,
    )
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComFerretDBAutoscaler/spec/${autoscaleType}/sentinel`,
