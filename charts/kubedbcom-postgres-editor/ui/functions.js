...
export const useFunc = (model) => {
  ...
  *************************      common functions ********************************************
  ...
  function showNewSecretCreateField() {
    const resp = !showAuthSecretField() && !showAuthPasswordField()
  ...
  async function getStorageClassNames() {
    ...
    if (!initialStorageClass) setStorageClass()
    return resources
  }
  ...
  function initPrePopulateDatabase() {
  ...
  function showScriptOrStashForm(value) {
    const dataSource = getValue(discriminator, '/dataSource')
    watchDependency('discriminator#/dataSource')
    return dataSource === value
  }

  function showConfigMapOrSecretName(value) {
    const sourceVolumeType = getValue(discriminator, '/sourceVolumeType')
  ...
  function showRepositorySelectOrCreate(value) {
  ...
  function showRuntimeForm(value) {
    const customizeRestoreJobRuntimeSettings = getValue(
      discriminator,
      '/customizeRestoreJobRuntimeSettings',
    )
  ...
  function showInvokerForm(value) {
    const backupInvoker = getValue(discriminator, '/backupInvoker')
  ...
  function setInitialRestoreSessionRepo() {
  ...
  function onRepositoryNameChange() {
  ...
  function initFromAnnotationValue(key) {
  ...
  function initFromAnnotationKeyValue(prefix) {
    const annotations = getMongoAnnotations(getValue, model)
    const newOb = {}
  ...
  function isEqualToServiceMonitorType(rootModel, value) {
  ...
  function setSecretConfigNamespace() {
    watchDependency('model#/metadata/release/namespace')
    const namespace = getValue(model, '/metadata/release/namespace')
    return namespace
  }
  ...
  function setConfigurationFiles() {
  ...
  function getOpsRequestUrl(reqType, mode) {
    const cluster = storeGet('/route/params/cluster')
  ...
  function isBlueprintOption(value) {
  ...
  async function getBlueprints(backup) {
  ...
  async function fetchNames(version, type, discriminatorName) {
  ...
  function onInputChange(modelPath, field, subfield, discriminatorName) {
  ...
  function setFileValueFromStash(modelPath, field, subfield, value) {
    const backends = getValue(model, modelPath)
    if (field !== 'encryptionSecret') backends[0][field][subfield] = value
    else backends[0]['repositories'][0][field][subfield] = value
    commit('wizard/model$update', {
  ...
  function setInitSchedule(modelPath, value) {
    const session = getValue(model, modelPath)
    session[0].scheduler.schedule = value
    commit('wizard/model$update', {
      path: modelPath,
  ...
  function getDefault(modelPath, field, subfield) {
    const backends = getValue(model, modelPath)
  ...
  async function dbTypeEqualsTo(mongoType, type) {
  ...
  function ifScalingTypeEqualsTo(type) {
    watchDependency('discriminator#/autoscalingType')
    watchDependency('model#/resources/autoscalingKubedbComPostgresAutoscaler/spec/databaseRef/name')
  ...
  function clearSpecModel(verd) {
    // This function is referenced but not defined in old-function.js
