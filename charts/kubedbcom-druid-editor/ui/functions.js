...
function isDedicatedModeSelected({ model, getValue, watchDependency }) {
  watchDependency('model#/resources/kubedbComDruid/spec/topology')
  isDedicatedSelected = getValue(model, '/resources/kubedbComDruid/spec/topology')
...
function isAuthPluginNotSearchGuard({ discriminator, getValue, watchDependency, commit }) {
  ...
  if (!ret) {
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/dataWarm')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/dataHot')
  }
  return ret
}
...
async function showInternalUsersAndRolesMapping({
  ...
  watchDependency('model#/resources/kubedbComDruid/spec/disableSecurity')
  watchDependency('model#/resources/kubedbComDruid/spec/version')
...
async function showSecureCustomConfig({
  ...
  if (ret) {
    ...
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/secureConfigSecret')
...
async function getDruidVersions({ axios, storeGet }, group, version, resource) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  ...
  const filteredDruidVersions = resources.filter((item) => item.spec && !item.spec.deprecated)

  filteredDruidVersions.map((item) => {
    const name = (item.metadata && item.metadata.name) || ''
  ...
  return filteredDruidVersions
}

function isSecurityEnabled({ model, getValue, watchDependency }) {
  watchDependency('model#/resources/kubedbComDruid/spec/disableSecurity')
  const value = getValue(model, '/resources/kubedbComDruid/spec/disableSecurity')
  return !value
}

function onDisableSecurityChange({ model, getValue, commit }) {
  const disableSecurity = getValue(model, '/resources/kubedbComDruid/spec/disableSecurity')
  ...
  if (disableSecurity) {
    ...
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/internalUsers')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/rolesMapping')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/tls')
  }
}
...
async function onVersionChange({
  ...
  if (!isOpenDistro && !isSearchGuard) {
    ...
    if (!isOpenDistro) {
      const internalUsers = getValue(model, '/resources/kubedbComDruid/spec/internalUsers')
      ...
      commit('wizard/model$update', {
        path: '/resources/kubedbComDruid/spec/internalUsers',
        value: internalUsers,
        force: true,
      })
      commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/dataHot')
    ...
    if (!isXpack) {
      commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/dataCold')
      commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/dataContent')
      commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/dataFrozen')
      commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/ml')
      commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology/transform')
    }
  }
}

function onEnableSSLChange({ model, getValue, commit }) {
  const enabelSSL = getValue(model, '/resources/kubedbComDruid/spec/enableSSL')
...
function setStorageClass({ model, getValue, commit }, path) {
  const deletionPolicy = getValue(model, 'resources/kubedbComDruid/spec/deletionPolicy') || ''
...
function deleteDatabaseModePath({ discriminator, getValue, commit }) {
  const mode = getValue(discriminator, '/activeDatabaseMode')
  if (mode === 'Dedicated') {
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/replicas')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/storage')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/maxUnavailable')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/podTemplate')
  } else if (mode === 'Combined') {
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/topology')
  }
}
...
function getMaxUnavailableOptions({ model, getValue, watchDependency, commit, elementUi }, path) {
  ...
  if (path) {
    ...
    const replacedPath = ($ref || '').replace(
      'schema#/properties/resources/properties/kubedbComDruid/properties/spec/properties/topology/properties/',
      '',
    )
    const dyn = replacedPath.split('/').shift()
    prefixPath = `/resources/kubedbComDruid/spec/topology/${dyn}`
  }
...
async function getSelectedVersionAuthPlugin(
  ...
  watchDependency('model#/resources/kubedbComDruid/spec/version')
  const version = getValue(model, '/resources/kubedbComDruid/spec/version') || ''

  const elasticVersions = await getDruidVersions(
    { axios, storeGet },
    'catalog.kubedb.com',
    'v1alpha1',
    'druidversions',
...
function onNodeSwitchFalse({ elementSchema, commit }) {
  ...
  commit('wizard/model$delete', `/resources/kubedbComDruid/spec/topology/${node}`)
}

function hasTopologyNode({ model, getValue, itemCtx }) {
  const nodeValue = getValue(model, `/resources/kubedbComDruid/spec/topology/${itemCtx}`)
...
function onInternalUsersChange({ discriminator, getValue, commit }) {
  ...
  if (Object.keys(internalUsers).length) {
    ...
    if (users) commit('wizard/model$delete', '/resources/kubedbComDruid/spec/internalUsers')
  }
}

function setInternalUsers({ model, getValue, watchDependency, setDiscriminatorValue }) {
  watchDependency('model#/resources/kubedbComDruid/spec/internalUsers')
  const internalUsers = getValue(model, '/resources/kubedbComDruid/spec/internalUsers')
...
function onRolesMappingChange({ discriminator, getValue, commit }) {
  ...
  if (Object.keys(rolesMapping).length) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComDruid/spec/rolesMapping',
      value: rolesMapping,
      force: true,
    })
  } else {
    ...
    if (roles) commit('wizard/model$delete', '/resources/kubedbComDruid/spec/rolesMapping')
  }
}

function setRolesMapping({ model, getValue, watchDependency, setDiscriminatorValue }) {
  watchDependency('model#/resources/kubedbComDruid/spec/rolesMapping')
  const rolesMapping = getValue(model, '/resources/kubedbComDruid/spec/rolesMapping')
...
function onCustomizeKernelSettingChange({ discriminator, getValue, commit }) {
  ...
  if (customizeKernelSettings === 'no') {
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/kernelSettings')
  } else if (customizeKernelSettings === 'disable') {
    commit('wizard/model$update', {
      path: '/resources/kubedbComDruid/spec/kernelSettings',
      value: {},
      force: true,
    })
  }
...
function setApiGroupEdit({ model, getValue }) {
  const kind = getValue(model, '/resources/kubedbComDruid/spec/tls/issuerRef/kind')
  const name = getValue(model, '/resources/kubedbComDruid/spec/tls/issuerRef/name')
...
async function getIssuerRefsName({ axios, storeGet, getValue, model, watchDependency }) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  watchDependency('model#/resources/kubedbComDruid/spec/tls/issuerRef/apiGroup')
  watchDependency('model#/resources/kubedbComDruid/spec/tls/issuerRef/kind')
  watchDependency('model#/metadata/release/namespace')
  const apiGroup = getValue(model, '/resources/kubedbComDruid/spec/tls/issuerRef/apiGroup')
  const kind = getValue(model, '/resources/kubedbComDruid/spec/tls/issuerRef/kind')
  const namespace = getValue(model, '/metadata/release/namespace')
...
function setClusterAuthMode({ model, getValue }) {
  const val = getValue(model, '/resources/kubedbComDruid/spec/clusterAuthMode')
  return val || 'x509'
}

function setSSLMode({ model, getValue }) {
  const val = getValue(model, '/resources/kubedbComDruid/spec/sslMode')
...
function onTlsConfigureChange({ discriminator, getValue, commit }) {
  ...
  if (configureStatus) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComDruid/spec/tls',
      value: { issuerRef: {}, certificates: [] },
      force: true,
    })
  } else {
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/tls')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/clusterAuthMode')
...
async function getAliasOptions({
  ...
  watchDependency('model#/resources/kubedbComDruid/spec/enableSSL')
  watchDependency('model#/resources/kubedbComDruid/spec/monitor')

  const enableSSL = getValue(model, '/resources/kubedbComDruid/spec/enableSSL')
  const monitor = getValue(model, '/resources/kubedbComDruid/spec/monitor')
...
function disableInitializationSection({ model, getValue, watchDependency }) {
  const initialized = getValue(model, '/resources/kubedbComDruid/spec/init/initialized')
  watchDependency('model#/resources/kubedbComDruid/spec/init/initialized')
  return !!initialized
}
...
function initPrePopulateDatabase({ getValue, model }) {
  const waitForInitialRestore = getValue(
    model,
    '/resources/kubedbComDruid/spec/init/waitForInitialRestore',
  )
  const stashAppscodeComRestoreSession_init = getValue(
    model,
    '/resources/stashAppscodeComRestoreSession_init',
  )
  const script = getValue(model, '/resources/kubedbComDruid/spec/init/script')
...
function onPrePopulateDatabaseChange({ commit, getValue, discriminator, model }) {
  ...
  if (prePopulateDatabase === 'no') {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComDruid/spec/init/waitForInitialRestore',
      value: false,
    })
    commit('wizard/model$delete', '/resources/stashAppscodeComRestoreSession_init')
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/init/script')
...
function onDataSourceChange({ commit, getValue, discriminator, model }) {
  ...
  if (dataSource === 'script') {
    ...
    if (!valueExists(model, getValue, '/resources/kubedbComDruid/spec/init/script'))
      commit('wizard/model$update', {
        path: '/resources/kubedbComDruid/spec/init/script',
        value: initScript,
      })
...
function initVolumeType({ getValue, model }) {
  const configMap = getValue(model, '/resources/kubedbComDruid/spec/init/script/configMap/name')
  const secret = getValue(model, '/resources/kubedbComDruid/spec/init/script/secret/secretName')
...
function onVolumeTypeChange({ commit, getValue, discriminator, model }) {
  ...
  if (sourceVolumeType === 'configMap') {
    ...
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/init/script/secret')

    if (!valueExists(model, getValue, '/resources/kubedbComDruid/spec/init/script/configMap')) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComDruid/spec/init/script/configMap',
        value: {
          name: '',
        },
      })
    ...
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/init/script/configMap')

    if (!valueExists(model, getValue, '/resources/kubedbComDruid/spec/init/script/secret')) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComDruid/spec/init/script/secret',
        value: {
          secretName: '',
        },
      })
...
function deleteKubedbComDruidDbAnnotation(getValue, model, commit) {
...
function addKubedbComDruidDbAnnotation(getValue, model, commit, key, value, force) {
  const annotations = getValue(model, '/resources/kubedbComDruid/metadata/annotations') || {}
...
function onScheduleBackupChange({ commit, getValue, discriminator, model }) {
  ...
  if (scheduleBackup === 'no') {
    ...
    deleteKubedbComDruidDbAnnotation(getValue, model, commit)
  } else {
...
async function setBackupSwitch({ commit, storeGet, axios, getValue, model }) {
  ...
  if (!isBackupOn) {
    ...
    try {
      ...
      dbResource = resp.data?.resources?.kubedbComDruid
    } catch (e) {
      console.log(e)
    }
    commit('wizard/model$update', {
  ...
  } else dbResource = getValue(model, '/resources/kubedbComDruid')
...
function onTaskParametersChange({ getValue, discriminator, model, commit }) {
  ...
  const oldAnnotations = getValue(model, '/resources/kubedbComDruid/metadata/annotations') || {}
...
function onNameChange({ commit, model, getValue }) {
  ...
  if (hasSecretConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComDruid/spec/configSecret/name',
      value: `${dbName}-config`,
      force: true,
    })
  }
...
function onSetCustomConfigChange({ discriminator, getValue, commit }) {
  ...
  if (value === 'no') {
    commit('wizard/model$delete', '/resources/kubedbComDruid/spec/configSecret')
    commit('wizard/model$delete', '/resources/secret_user_config')
  }
}

function initSetCustomConfig({ model, getValue }) {
  const configSecret = getValue(model, '/resources/kubedbComDruid/spec/configSecret/name')
...
function onSecretConfigurationSourceChange({ getValue, discriminator, commit, model }) {
  ...
  if (configurationSource === 'use-existing-config') {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComDruid/spec/secureConfigSecret/name',
      value: configSecretName,
      force: true,
    })
  }
...
function initSetSecureCustomConfig({ model, getValue }) {
  const configSecret = getValue(model, '/resources/kubedbComDruid/spec/secureConfigSecret/name')
...
function getOpsRequestUrl({ storeGet, model, getValue, mode }, reqType) {
  ...
  if (mode === 'standalone-step')
    ...
    return `${domain}/${owner}/kubernetes/${cluster}/ops.kubedb.com/v1alpha1/druidopsrequests/create?name=${dbname}&namespace=${namespace}&group=${group}&version=${version}&resource=${resource}&kind=${kind}&page=operations&requestType=${reqType}`
}
...
return {
  ...
  deleteKubedbComDruidDbAnnotation,
  addKubedbComDruidDbAnnotation,
  initScheduleBackup,
