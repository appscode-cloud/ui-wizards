...
export const useFunc = (model) => {
   ...
   */
  function getMachines(nodeType, minmax) {
    // watchDependency('discriminator#/topologyMachines')
    const depends = minmax === 'min' ? 'max' : 'min'
    const dependantPath = `/allowedMachine-${nodeType}-${depends}`
  ...
  function setAllowedMachine(nodeType, minmax) {
    ...
    const machine = parsedInstance[nodeType] || ''
  ...
  function onMachineChange(nodeType) {
    ...
    const minMachine = getValue(discriminator, `/allowedMachine-${nodeType}-min`)
    const maxMachine = getValue(discriminator, `/allowedMachine-${nodeType}-max`)
  ...
  function setControlledResources(nodeType) {
    const list = ['cpu', 'memory']
    const path = `/resources/autoscalingKubedbComDruidAutoscaler/spec/compute/${nodeType}/controlledResources`
    commit('wizard/model$update', {
      path: path,
      value: list,
      force: true,
