...
export const useFunc = (model) => {
  ...
  function getBackupConfigsAndAnnotations(getValue, model) {
    ...
    const kubedbComDocumentDBAnnotations =
      getValue(model, '/resources/kubedbComDocumentDB/metadata/annotations') || {}

    const isBluePrint = Object.keys(kubedbComDocumentDBAnnotations).some(
      (k) =>
  ...
  function deleteKubeDbComMySqlDbAnnotation(getValue, model, commit) {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComDocumentDB/metadata/annotations',
  ...
  let valuesFromWizard = {
    ...
    spec: {
      backends: [
        {
          name: 'documentdb-backend',
          retentionPolicy: {
            name: 'documentdb-retention-policy',
            namespace: 'demo',
          },
          storageRef: {
      ...
      sessions: [
        {
          ...
          name: 'documentdb-frequent-backup',
          repositories: [
            {
              backend: 'documentdb-backend',
              directory: '/documentdb-repo',
              encryptionSecret: {
                name: 'documentdb-encryption-secret',
                namespace: 'demo',
              },
              name: 'documentdb-repo',
            },
          ],
          scheduler: {
  ...
  async function initBackupData() {
    ...
    dbResource = getValue(model, '/resources/kubedbComDocumentDB')
  ...
  function onBackupTypeChange() {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComDocumentDB',
  ...
  function onArchiverChange() {
    ...
    const path = 'resources/kubedbComDocumentDB/spec/archiver'
    if (archiverSwitch) {
      commit('wizard/model$update', {
        path: path,
  ...
  function addLabelAnnotation(type) {
    ...
    commit('wizard/model$update', {
      path: `/resources/kubedbComDocumentDB/metadata/${type}`,
      value: obj,
      force: true,
    })
  }
  ...
  function isConsole() {
    ...
    if (isKube) {
      ...
      commit('wizard/model$update', {
        path: '/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/databaseRef/name',
        value: dbName,
        force: true,
      })
      ...
      commit('wizard/model$update', {
        path: '/resources/autoscalingKubedbComDocumentDBAutoscaler/metadata/name',
        value: modifiedName,
        force: true,
      })
      const namespace = storeGet('/route/query/namespace') || ''
      if (namespace) {
        commit('wizard/model$update', {
          path: '/resources/autoscalingKubedbComDocumentDBAutoscaler/metadata/namespace',
          value: namespace,
          force: true,
        })
      }
  ...
  function onNamespaceChange() {
    ...
    const agent = getValue(model, '/resources/kubedbComDocumentDB/spec/monitor/agent')
  ...
  async function getMysqlDbs() {
    ...
    const namespace = getValue(
      model,
      '/resources/autoscalingKubedbComDocumentDBAutoscaler/metadata/namespace',
    )
    const owner = storeGet('/route/params/user')
    const cluster = storeGet('/route/params/cluster')
    const storageEngine = getValue(model, '/resources/kubedbComDocumentDB/spec/storageEngine')
  ...
  function initMetadata() {
    const dbName =
      getValue(
        model,
        '/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/databaseRef/name',
      ) || ''
    ...
    if (type === 'compute')
      commit(
        'wizard/model$delete',
        '/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/storage',
      )
    if (type === 'storage')
      commit(
        'wizard/model$delete',
        '/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/compute',
      )
  }

  async function fetchTopologyMachines() {
    const annotations =
      getValue(model, '/resources/autoscalingKubedbComDocumentDBAutoscaler/metadata/annotations') ||
      {}
  ...
  function onTriggerChange(type) {
    ...
    const commitPath = `/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/${type}/trigger`
  ...
  function onMachineChange(type) {
    const annoPath = '/resources/autoscalingKubedbComDocumentDBAutoscaler/metadata/annotations'
    ...
    const maxMachineAllocatable = maxMachineObj
      ...
      : null
    const allowedPath = `/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/compute/${type}`
  ...
  function setControlledResources(type) {
    ...
    const path = `/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/compute/${type}/controlledResources`
    commit('wizard/model$update', {
      path: path,
      value: list,
      force: true,
  ...
  function isNodeTopologySelected() {
    // watchDependency(
    //   'model#/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/compute/nodeTopology/name',
    // )
    const nodeTopologyName =
      getValue(
        model,
        '/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/compute/nodeTopology/name',
      ) || ''
  ...
  function showOpsRequestOptions() {
    ...
    // watchDependency('model#/resources/autoscalingKubedbComDocumentDBAutoscaler/spec/databaseRef/name')
  ...
  function onEnableMonitoringChange() {
    ...
    if (configureStatus) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComDocumentDB/spec/monitor',
        value: {},
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/kubedbComDocumentDB/spec/monitor')
    }
  ...
  function onCustomizeExporterChange() {
    ...
    if (configureStatus) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter',
        value: {},
        force: true,
      })
    } else {
      commit(
        'wizard/model$delete',
        '/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter',
      )
    }
  }
  ...
  function onLabelChange() {
    const labels = getValue(model, '/resources/kubedbComDocumentDB/spec/metadata/labels')
  ...
  function getOpsRequestUrl(reqType) {
    ...
    if (isKube) return pathConstructedForKubedb
      ...
      return `${domain}/console/${owner}/kubernetes/${cluster}/ops.kubedb.com/v1alpha1/documentdbopsrequests/create?name=${dbname}&namespace=${namespace}&group=${group}&version=${version}&resource=${resource}&kind=${kind}&page=operations&requestType=VerticalScaling`
  }
  ...
  async function getConfigMapKeys(index) {
    ...
    const configMapName = getValue(
      model,
      `/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter/env/${index}/valueFrom/configMapKeyRef/name`,
    )
  ...
  async function getSecretKeys(index) {
    ...
    const secretName = getValue(
      model,
      `/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter/env/${index}/valueFrom/secretKeyRef/name`,
    )
  ...
  function isBindingAlreadyOn() {
    ...
    const isExposeBinding = !!keys.find((str) => str === 'catalogAppscodeComDocumentDBBinding')
  ...
  function addOrRemoveBinding() {
    ...
    const labels = getValue(model, '/resources/kubedbComDocumentDB/metadata/labels')
    const bindingValues = {
    ...
    if (value) {
      commit('wizard/model$update', {
        path: '/resources/catalogAppscodeComDocumentDBBinding',
        value: bindingValues,
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/catalogAppscodeComDocumentDBBinding')
    }
  }
  ...
  function onEnvArrayChange() {
    ...
    if (filteredEnv.length)
      commit('wizard/model$update', {
        path: '/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter/env',
        value: filteredEnv,
        force: true,
      })
  }
  ...
  function initEnvArray() {
    const env = getValue(
      model,
      '/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter/env',
    )
  ...
  function initMonitoring() {
    const env =
      getValue(model, '/resources/kubedbComDocumentDB/spec/monitor/prometheus/exporter/env') || []
