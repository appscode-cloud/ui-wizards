...
async function getDbDetails({
  ...
  if (namespace && name) {
    const resp = await axios.get(
      `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/clickhouses/${name}`,
    )
...
async function getDbVersions({ axios, storeGet, getValue, discriminator }) {
  ...
  try {
    ...
    const presetVersions = presets.admin?.databases?.ClickHouse?.versions?.available || []
    const queryParams = {
      filter: {
        items: {
          metadata: { name: null },
...
function isVerticalScaleTopologyRequired({ watchDependency, getValue, discriminator, commit }) {
  ...
  const path = `/spec/verticalScaling/clickhouse/topology`
