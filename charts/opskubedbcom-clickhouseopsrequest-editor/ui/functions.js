...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/clickhouses/${name}`
      const resp = await axios.get(url)

      setDiscriminatorValue('/dbDetails', resp.data || {})
      if (route.params.actions === 'scale-storage') {
        storageClassName = resp.data?.spec?.storage?.storageClassName || ''
        if (storageClassName) {
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.ClickHouse?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const resp = await axios.get(
        `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/clickhouseversions`,
        {
          params: queryParams,
        },
      )
      ...
      const filteredClickHouseVersions = sortedVersions.filter((item) => {
      ...
      setDiscriminatorValue('/filteredVersion', filteredClickHouseVersions)

      return filteredClickHouseVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
  ...
  function setMachine() {
    ...
    const type = dbDetails?.spec?.clusterTopology ? 'cluster' : 'standalone'
  ...
  function getLimits() {
    const dbDetails = getValue(discriminator, '/dbDetails')
    const isTopology = !!dbDetails?.spec?.clusterTopology

    const spec = isTopology
      ? dbDetails?.spec?.clusterTopology?.cluster?.podTemplate?.spec
      : dbDetails?.spec?.podTemplate?.spec

    const containers = spec?.containers ?? []

    if (containers.length === 0) {
      return spec?.resources?.requests ?? {}
    }
