...
async function getDbVersions({ axios, storeGet, getValue, discriminator }) {
  ...
  try {
    const presetVersions = presets.admin?.databases?.Singlestore?.versions?.available || []
    const queryParams = {
      filter: {
        items: {
          metadata: { name: null },
...
function setMachine({ getValue, discriminator, storeGet }, type) {
  ...
  let machine = ''
  if (type === 'aggregator' || type === 'node') machine = annotations['kubernetes.io/instance-type']
