...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/pgpools/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.Pgpool?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const versionsToShow = filteredPgpoolVersions.length
        ? filteredPgpoolVersions
