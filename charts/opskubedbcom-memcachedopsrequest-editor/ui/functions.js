...
export const useFunc = (model) => {
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.Memcached?.versions?.available || []
      if (presetVersions.length === 0) {
        const resp = await axios.get(
          `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/all-available`,
        )
        const versions = resp.data?.MemcachedVersion || []
        return versions
      }
      const queryParams = {
        filter: {
      ...
      setDiscriminatorValue('/filteredVersion', filteredMemcachedVersions)
