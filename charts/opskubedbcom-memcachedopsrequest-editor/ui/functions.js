...
export const useFunc = (model) => {
  ...
  async function getDbs() {
    ...
    const resp = await axios.get(
      `/clusters/${owner}/${cluster}/proxy/kubedb.com/v1alpha2/namespaces/${namespace}/memcacheds`,
      {
        params: { filter: { items: { metadata: { name: null } } } },
      },
    )
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/v1alpha2/namespaces/${namespace}/memcacheds/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      const presetVersions = presets.admin?.databases?.Memcached?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const resp = await axios.get(
        `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/memcachedversions`,
        {
          params: queryParams,
        },
      )
      ...
      const filteredMemcachedVersions = sortedVersions.filter((item) => {
      ...
      return filteredMemcachedVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
  ...
  function onMachineChange(type, valPath) {
    ...
    if (selectedMachine.machine === 'custom') parsedInstance = undefined
    else parsedInstance = selectedMachine.machine
    ...
    if (
      !parsedInstance ||
      (typeof parsedInstance === 'object' && Object.keys(parsedInstance).length === 0)
    )
  ...
  function isVerticalScaleTopologyRequired() {
    ...
    const path = `/spec/verticalScaling/memcached/topology`
