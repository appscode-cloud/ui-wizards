...
// ************************* Basic Info **********************************************
async function getMariaDbVersions({ axios, storeGet }, group, version, resource) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
...
function setStorageClass({ model, getValue, commit }) {
  ...
  let storageClass =
    getValue(model, '/resources/kubedbComMariaDB/spec/storage/storageClassName') || ''
  const suffix = '-retain'
...
async function getIssuerRefsName({ axios, storeGet, getValue, model, watchDependency }) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  watchDependency('model#/resources/kubedbComMariaDB/spec/tls/issuerRef/apiGroup')
  watchDependency('model#/resources/kubedbComMariaDB/spec/tls/issuerRef/kind')
  watchDependency('model#/metadata/release/namespace')
  const apiGroup = getValue(model, '/resources/kubedbComMariaDB/spec/tls/issuerRef/apiGroup')
  const kind = getValue(model, '/resources/kubedbComMariaDB/spec/tls/issuerRef/kind')
  const namespace = getValue(model, '/metadata/release/namespace')
...
function initPrePopulateDatabase({ getValue, model }) {
  ...
  const script = getValue(model, '/resources/kubedbComMariaDB/spec/init/script')
...
function onPrePopulateDatabaseChange({ commit, getValue, discriminator, model }) {
  ...
  if (prePopulateDatabase === 'no') {
    ...
    commit('wizard/model$delete', '/resources/kubedbComMariaDB/spec/init/script')
...
function onDataSourceChange({ commit, getValue, discriminator, model }) {
  ...
  if (dataSource === 'script') {
    ...
    if (!valueExists(model, getValue, '/resources/kubedbComMariaDB/spec/init/script'))
      commit('wizard/model$update', {
...
function initVolumeType({ getValue, model }) {
  const configMap = getValue(model, '/resources/kubedbComMariaDB/spec/init/script/configMap/name')
  const secret = getValue(model, '/resources/kubedbComMariaDB/spec/init/script/secret/secretName')
...
function onVolumeTypeChange({ commit, getValue, discriminator, model }) {
  ...
  if (sourceVolumeType === 'configMap') {
    ...
    commit('wizard/model$delete', '/resources/kubedbComMariaDB/spec/init/script/secret')

    if (!valueExists(model, getValue, '/resources/kubedbComMariaDB/spec/init/script/configMap')) {
      commit('wizard/model$update', {
    ...
    commit('wizard/model$delete', '/resources/kubedbComMariaDB/spec/init/script/configMap')

    if (!valueExists(model, getValue, '/resources/kubedbComMariaDB/spec/init/script/secret')) {
      commit('wizard/model$update', {
...
function addKubeDbComMariaDbAnnotation(getValue, model, commit, key, value, force) {
  const annotations = getValue(model, '/resources/kubedbComMariaDB/metadata/annotations') || {}
...
function onTaskParametersChange({ getValue, discriminator, model, commit }) {
  ...
  const oldAnnotations = getValue(model, '/resources/kubedbComMariaDB/metadata/annotations') || {}
...
function getCreateAuthSecret({ model, getValue }) {
  const authSecret = getValue(model, '/resources/kubedbComMariaDB/spec/authSecret')
...
function disableInitializationSection({ model, getValue, watchDependency }) {
  const initialized = getValue(model, '/resources/kubedbComMariaDB/spec/init/initialized')
...
function onCreateAuthSecretChange({ discriminator, getValue, commit }) {
  ...
  if (createAuthSecret) {
    commit('wizard/model$delete', '/resources/kubedbComMariaDB/spec/authSecret')
...
async function getBlueprints({ getValue, model, setDiscriminatorValue, axios, storeGet }, backup) {
  ...
  const namespace = getValue(model, '/metadata/release/namespace') || ''
  const url = `clusters/${username}/${clusterName}/proxy/core.kubestash.com/v1alpha1/namespaces/${namespace}/backupblueprints`
...
function onInputChange(
  ...
  const invokerName = getValue(discriminator, '/backupInvoker')
  let commitPath = modelPath
  if (invokerName === 'backupBlueprint') {
    commitPath = commitPath.replace(
      'resources/coreKubestashComBackupConfiguration/spec/',
      'resources/coreKubestashComBackupBlueprint/spec/backupConfigurationTemplate/',
    )
  }
  const value = getValue(discriminator, `/${discriminatorName}`)

  const backends = getValue(model, commitPath) || []
...
function onInputChangeSchedule(
  ...
  const value = getValue(discriminator, `/${discriminatorName}`)
  const session = getValue(model, commitPath)
...
function getDefault(
  ...
  const backends = getValue(model, path)
...
function getDefaultSchedule(
  ...
  const session = getValue(model, path)
  return session[0]?.scheduler.schedule
}

function onBackupChange({ discriminator, getValue, commit, storeGet }) {
...
async function setBackupSwitch({ commit, storeGet, axios }) {
  ...
  if (filteredBlueprint) {
    try {
      ...
      blueprintDetails = resp.data
    } catch (e) {
      console.log(e)
    }
  } else {
    try {
      const actions = storeGet('/resource/actions/result') || []
      const chart = actions?.[0].items?.[0].editor
      const chartName = chart.name
      const chartVersion = chart.version
      const sourceRef = chart.sourceRef
      const {
...
function isAllowedNamespacesEqualTo({ getValue, model, watchDependency }, type) {
  watchDependency(
    'model#/resources/coreKubestashComBackupBlueprint/spec/usagePolicy/allowedNamespaces/from',
  )
  const allowedNamespace = getValue(
    model,
    '/resources/coreKubestashComBackupBlueprint/spec/usagePolicy/allowedNamespaces/from',
  )
