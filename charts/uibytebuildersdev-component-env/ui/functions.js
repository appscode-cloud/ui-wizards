...
function isInputTypeValueFrom({ rootModel }) {
  return !isConfigMapTypeValueFrom({ rootModel }) && !isSecretTypeValueFrom({ rootModel })
}

function onValueFromChange({ rootModel, discriminator, getValue, updateModelValue }) {
  const valueFrom = getValue(discriminator, '/valueFromType')
  if (valueFrom === 'input') {
...
async function getSecrets({ storeGet, axios, getValue, watchDependency, reusableElementCtx }) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  const namespace = getValue(reusableElementCtx, '/dataContext/namespace')
