...
function onNameChange({ commit, model, getValue, discriminator }) {
  ...
  const hasSecretUnsealerCreds = getValue(model, '/resources/secret_unsealer_creds')
...
async function getVaultServerVersions({ axios, storeGet }, group, version, resource) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
...
function isLowAvailableStorageBackendSelected({ model, getValue, watchDependency, discriminator }) {
    ...
    backends.forEach((item) => {
      if (item !== selectedBackend) {
        ...
        backendSecretObj[item]?.secretObjectDataKeys.forEach((key) => {
          commit('wizard/model$delete', `/resources/secret_backend_creds/stringData/${key}`)
        })
      } else {
...
function getVaultBackendType({ model, getValue, discriminator }) {
  ...
  if (Object.keys(backend).length) return Object.keys(backend).find((key) => key)
  else {
...
function setCredSecretData({
  ...
  const data = getValue(model, '/resources/secret_backend_creds/stringData') || {}
  ...
  unsealerModes.forEach((item) => {
    if (item && item !== selectedMode) {
      ...
      unsealerSecretObj[item]?.secretObjectDataKeys.forEach((key) => {
        commit('wizard/model$delete', `/resources/secret_unsealer_creds/stringData/${key}`)
      })
    }
  })
...
function getUnsealerMode({ model, getValue, discriminator }) {
  ...
  if (Object.keys(unsealerMode).length) return Object.keys(unsealerMode).find((key) => key)
  else {
...
function onCreateUnsealerTlsSecretChange({ discriminator, model, getValue, commit }) {
  const createTlsSecretStatus = getValue(discriminator, '/createTlsSecret')
  const vsName = getValue(model, '/metadata/release/name')
...
function showUnsealerExisitingTlsSecretField({ discriminator, getValue, watchDependency }) {
  watchDependency('discriminator#/createTlsSecret')
  const createTlsSecretStatus = getValue(discriminator, '/createTlsSecret')
  return !createTlsSecretStatus
}

function showUnsealerNewTlsSecretField({ discriminator, getValue, watchDependency }) {
...
function setUnsealerTlsSecretClientCertPassword({ discriminator, model, getValue }) {
  const createTlsSecretStatus = getValue(discriminator, '/createTlsSecret')
  const clientCertValue = getValue(model, '/resources/secret_unsealer_tls')
...
function onUnsealerTlsClientCertPasswordChange({ discriminator, getValue, commit }) {
  const createTlsSecretStatus = getValue(discriminator, '/createTlsSecret')
...
async function getIssuerRefsName({ axios, storeGet, getValue, model, watchDependency }) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  watchDependency('model#/resources/kubevaultComVaultServer/spec/tls/issuerRef/apiGroup')
  watchDependency('model#/resources/kubevaultComVaultServer/spec/tls/issuerRef/kind')
  watchDependency('model#/metadata/release/namespace')
  const apiGroup = getValue(model, '/resources/kubevaultComVaultServer/spec/tls/issuerRef/apiGroup')
  const kind = getValue(model, '/resources/kubevaultComVaultServer/spec/tls/issuerRef/kind')
  const namespace = getValue(model, '/metadata/release/namespace')
