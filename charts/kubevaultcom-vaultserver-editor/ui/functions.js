...
function initPrePopulateDatabase({ getValue, model }) {
  const waitForInitialRestore = getValue(
    model,
    "/resources/kubevaultComVaultServer/spec/init/waitForInitialRestore"
  );
  ...
  const script = getValue(
    model,
    "/resources/kubevaultComVaultServer/spec/init/script"
  );
...
function onPrePopulateDatabaseChange({
  ...
  if (prePopulateDatabase === "no") {
    ...
    commit("wizard/model$update", {
      path: "/resources/kubevaultComVaultServer/spec/init/waitForInitialRestore",
      value: false,
    });
    commit(
      "wizard/model$delete",
...
function onDataSourceChange({ commit, getValue, discriminator, model }) {
  ...
  if (dataSource === "script") {
    ...
    if (
      ...
      commit("wizard/model$update", {
        path: "/resources/kubevaultComVaultServer/spec/init/script",
        value: initScript,
      });
...
function initVolumeType({ getValue, model }) {
  const configMap = getValue(
    model,
    "/resources/kubevaultComVaultServer/spec/init/script/configMap/name"
  );
  const secret = getValue(
    model,
    "/resources/kubevaultComVaultServer/spec/init/script/secret/secretName"
  );
...
function onVolumeTypeChange({ commit, getValue, discriminator, model }) {
  ...
  if (sourceVolumeType === "configMap") {
    ...
    commit(
      "wizard/model$delete",
      "/resources/kubevaultComVaultServer/spec/init/script/secret"
    );
    ...
    if (
      !valueExists(
        model,
        getValue,
        "/resources/kubevaultComVaultServer/spec/init/script/configMap"
      )
    ) {
      commit("wizard/model$update", {
        path: "/resources/kubevaultComVaultServer/spec/init/script/configMap",
        value: {
          name: "",
        },
      });
    ...
    if (
      ...
      commit("wizard/model$update", {
        path: "/resources/kubevaultComVaultServer/spec/init/script/secret",
        value: {
          secretName: "",
        },
      });
...
function getBackupConfigsAndAnnotations(getValue, model) {
  ...
  const kubevaultComVaultServerAnnotations =
    getValue(model, "/resources/kubevaultComVaultServer/metadata/annotations") || {};

  const isBluePrint = Object.keys(kubevaultComVaultServerAnnotations).some(
    (k) =>
...
function deletekubevaultComVaultServerAnnotation(getValue, model, commit) {
  const annotations =
    getValue(model, "/resources/kubevaultComVaultServer/metadata/annotations") || {};
  ...
  commit("wizard/model$update", {
    path: "/resources/kubevaultComVaultServer/metadata/annotations",
    value: filteredAnnotations,
  });
}

function addkubevaultComVaultServerAnnotation(
  getValue,
  model,
  commit,
  key,
...
function onScheduleBackupChange({
  ...
  if (scheduleBackup === "no") {
    ...
    // delete annotation from kubevaultComVaultServer annotation
    deletekubevaultComVaultServerAnnotation(getValue, model, commit);
  } else {
...
function onBackupInvokerChange({
  ...
  if (backupInvoker === "backupConfiguration") {
    ...
    addkubevaultComVaultServerAnnotation(
      getValue,
      model,
      commit,
...
function getMongoAnnotations(getValue, model) {
  const annotations = getValue(
    model,
    "/resources/kubevaultComVaultServer/metadata/annotations"
  );
  return { ...annotations } || {};
}
...
function onNamespaceChange({ commit, model, getValue }) {
  ...
  const agent = getValue(
    model,
    "/resources/kubevaultComVaultServer/spec/monitor/agent"
  );
...
function onLabelChange({ commit, model, getValue }) {
  const labels = getValue(
    model,
    "/resources/kubevaultComVaultServer/spec/metadata/labels"
  );
...
function onNameChange({ commit, model, getValue }) {
  ...
  if (hasSecretShardConfig) {
    commit("wizard/model$update", {
      path:
        "/resources/kubevaultComVaultServer/spec/shardTopology/shard/configSecret/name",
  ...
  if (hasSecretConfigServerConfig) {
    commit("wizard/model$update", {
      path:
        "/resources/kubevaultComVaultServer/spec/shardTopology/configServer/configSecret/name",
  ...
  if (hasSecretMongosConfig) {
    commit("wizard/model$update", {
      path:
        "/resources/kubevaultComVaultServer/spec/shardTopology/mongos/configSecret/name",
...
return {
  ...
  getAliasOptions,
	...
	deletekubevaultComVaultServerAnnotation,
	addkubevaultComVaultServerAnnotation,
	initScheduleBackup,
