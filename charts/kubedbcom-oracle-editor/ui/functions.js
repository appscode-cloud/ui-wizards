...
export const useFunc = (model) => {
  ...
  function onScheduleBackupChange() {
    ...
    if (scheduleBackup === 'no') {
      ...
      // delete annotation from kubedbComOracle annotation
      deleteKubeDbComOracleDbAnnotation(getValue, model, commit)
    } else {
  ...
  function getBackupConfigsAndAnnotations(getValue, model) {
    ...
    const kubedbComOracleAnnotations =
      getValue(model, '/resources/kubedbComOracle/metadata/annotations') || {}

    const isBluePrint = Object.keys(kubedbComOracleAnnotations).some(
      (k) =>
  ...
  function deleteKubeDbComOracleDbAnnotation(getValue, model, commit) {
    const annotations = getValue(model, '/resources/kubedbComOracle/metadata/annotations') || {}
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComOracle/metadata/annotations',
  ...
  async function initBackupData() {
    ...
    dbResource = getValue(model, '/resources/kubedbComOracle')
  ...
  function onBackupTypeChange() {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComOracle',
  ...
  function onArchiverChange() {
    ...
    const path = 'resources/kubedbComOracle/spec/archiver'
    if (archiverSwitch) {
      commit('wizard/model$update', {
        path: path,
  ...
  function addLabelAnnotation(type) {
    ...
    commit('wizard/model$update', {
      path: `/resources/kubedbComOracle/metadata/${type}`,
      value: obj,
      force: true,
    })
  }
  ...
  function isConsole() {
    ...
    if (isKube) {
      ...
      commit('wizard/model$update', {
        path: '/resources/autoscalingKubedbComOracleAutoscaler/spec/databaseRef/name',
        value: dbName,
        force: true,
      })
      ...
      commit('wizard/model$update', {
        path: '/resources/autoscalingKubedbComOracleAutoscaler/metadata/name',
        value: modifiedName,
        force: true,
      })
      const namespace = storeGet('/route/query/namespace') || ''
      if (namespace) {
        commit('wizard/model$update', {
          path: '/resources/autoscalingKubedbComOracleAutoscaler/metadata/namespace',
          value: namespace,
          force: true,
        })
      }
  ...
  function onNamespaceChange() {
    ...
    const agent = getValue(model, '/resources/kubedbComOracle/spec/monitor/agent')
  ...
  async function getOracleDbs() {
    // watchDependency('model#/resources/autoscalingKubedbComOracleAutoscaler/metadata/namespace')
    const namespace = getValue(
      model,
      '/resources/autoscalingKubedbComOracleAutoscaler/metadata/namespace',
    )
    const owner = storeGet('/route/params/user')
    const cluster = storeGet('/route/params/cluster')
    const storageEngine = getValue(model, '/resources/kubedbComOracle/spec/storageEngine')
  ...
  function initMetadata() {
    const dbName =
      getValue(model, '/resources/autoscalingKubedbComOracleAutoscaler/spec/databaseRef/name') || ''
    ...
    if (type === 'compute')
      commit('wizard/model$delete', '/resources/autoscalingKubedbComOracleAutoscaler/spec/storage')
    if (type === 'storage')
      commit('wizard/model$delete', '/resources/autoscalingKubedbComOracleAutoscaler/spec/compute')
  }

  async function fetchTopologyMachines() {
    const annotations =
      getValue(model, '/resources/autoscalingKubedbComOracleAutoscaler/metadata/annotations') || {}
  ...
  function onTriggerChange(type) {
    ...
    const commitPath = `/resources/autoscalingKubedbComOracleAutoscaler/spec/${type}/trigger`
  ...
  function onMachineChange(type) {
    const annoPath = '/resources/autoscalingKubedbComOracleAutoscaler/metadata/annotations'
    ...
    const maxMachineAllocatable = maxMachineObj
      ...
      : null
    const allowedPath = `/resources/autoscalingKubedbComOracleAutoscaler/spec/compute/${type}`
  ...
  function setControlledResources(type) {
    ...
    const path = `/resources/autoscalingKubedbComOracleAutoscaler/spec/compute/${type}/controlledResources`
    commit('wizard/model$update', {
      path: path,
      value: list,
      force: true,
  ...
  function isNodeTopologySelected() {
    // watchDependency(
    //   'model#/resources/autoscalingKubedbComOracleAutoscaler/spec/compute/nodeTopology/name',
    // )
    const nodeTopologyName =
      getValue(
        model,
        '/resources/autoscalingKubedbComOracleAutoscaler/spec/compute/nodeTopology/name',
      ) || ''
  ...
  function showOpsRequestOptions() {
    ...
    // watchDependency('model#/resources/autoscalingKubedbComOracleAutoscaler/spec/databaseRef/name')
  ...
  function onEnableMonitoringChange() {
    ...
    if (configureStatus) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComOracle/spec/monitor',
        value: {},
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/kubedbComOracle/spec/monitor')
    }
  ...
  function onCustomizeExporterChange() {
    ...
    if (configureStatus) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComOracle/spec/monitor/prometheus/exporter',
        value: {},
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/kubedbComOracle/spec/monitor/prometheus/exporter')
    }
  }
  ...
  function onLabelChange() {
    const labels = getValue(model, '/resources/kubedbComOracle/spec/metadata/labels')
  ...
  function getOpsRequestUrl(reqType) {
    ...
    if (isKube) return pathConstructedForKubedb
      ...
      return `${domain}/console/${owner}/kubernetes/${cluster}/ops.kubedb.com/v1alpha1/oracleopsrequests/create?name=${dbname}&namespace=${namespace}&group=${group}&version=${version}&resource=${resource}&kind=${kind}&page=operations&requestType=VerticalScaling`
  }
  ...
  function onNamespaceChange() {
    ...
    if (!namespace) {
      commit(
        'wizard/model$delete',
        '/resources/autoscalingKubedbComOracleAutoscaler/spec/databaseRef/name',
      )
    }
  }
  ...
  async function getConfigMapKeys(index) {
    ...
    const configMapName = getValue(
      model,
      `/resources/kubedbComOracle/spec/monitor/prometheus/exporter/env/${index}/valueFrom/configMapKeyRef/name`,
    )
  ...
  async function getSecretKeys(index) {
    ...
    const secretName = getValue(
      model,
      `/resources/kubedbComOracle/spec/monitor/prometheus/exporter/env/${index}/valueFrom/secretKeyRef/name`,
    )
  ...
  function isBindingAlreadyOn() {
    ...
    const isExposeBinding = !!keys.find((str) => str === 'catalogAppscodeComOracleBinding')
  ...
  function addOrRemoveBinding() {
    ...
    const labels = getValue(model, '/resources/kubedbComOracle/metadata/labels')
    const bindingValues = {
    ...
    if (value) {
      commit('wizard/model$update', {
        path: '/resources/catalogAppscodeComOracleBinding',
        value: bindingValues,
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/catalogAppscodeComOracleBinding')
    }
  }
  ...
  function onEnvArrayChange() {
    ...
    if (filteredEnv.length)
      commit('wizard/model$update', {
        path: '/resources/kubedbComOracle/spec/monitor/prometheus/exporter/env',
        value: filteredEnv,
        force: true,
      })
  }
  ...
  function initEnvArray() {
    const env = getValue(model, '/resources/kubedbComOracle/spec/monitor/prometheus/exporter/env')
  ...
  function initMonitoring() {
    const env =
      getValue(model, '/resources/kubedbComOracle/spec/monitor/prometheus/exporter/env') || []
