...
async function getDbDetails({
  ...
  if (namespace && name) {
    const resp = await axios.get(
      `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/solrs/${name}`,
    )
...
async function getDbVersions({ axios, storeGet, getValue, discriminator }) {
  ...
  try {
    ...
    const presetVersions =
      presetResp.data?.spec?.values?.spec?.admin?.databases?.Solr?.versions?.available || []
