...
export const useFunc = (model) => {
    ...
    if (namespace && name) {
      ...
      if (route.params.actions === 'scale-storage') {
        storageClassName =
          ...
          resp.data?.spec?.topology?.coordinator?.storage?.storageClassName ||
          resp.data?.spec?.topology?.data?.storage?.storageClassName ||
          resp.data?.spec?.topology?.overseer?.storage?.storageClassName ||
          ''
        if (storageClassName) {
  ...
  async function getDbVersions() {
    ...
    try {
      ...
      const versionsToShow = filteredSolrVersions.length
        ? filteredSolrVersions
  ...
  function isReplicasValid(type) {
    ...
    const replicaTypes = ['coordinator', 'data', 'overseer']
