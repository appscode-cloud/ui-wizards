...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/kafkas/${name}`
      ...
      if (route.params.actions === 'scale-storage') {
        storageClassName =
          ...
          resp.data?.spec?.topology?.broker?.storage?.storageClassName ||
          resp.data?.spec?.topology?.controller?.storage?.storageClassName ||
          ''
        if (storageClassName) {
  ...
  async function getDbVersions() {
    ...
    try {
      ...
      const filteredKafkaVersions = sortedVersions.filter((item) => {
      ...
      const versionsToShow = filteredKafkaVersions.length
        ? filteredKafkaVersions
  ...
  function clearOpsReqSpec(verd, opsReqType) {
    if (
      ...
      } else if (verd === 'Combined') {
        commit('wizard/model$delete', `/spec/${opsReqType}/broker`)
        commit('wizard/model$delete', `/spec/${opsReqType}/controller`)
  ...
  function isReplicasValid(type) {
    ...
    const replicaTypes = ['broker', 'controller']
