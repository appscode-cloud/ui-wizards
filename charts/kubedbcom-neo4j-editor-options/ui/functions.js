...
export const useFunc = (model) => {
  ...
  async function initBundle() {
    ...
    if (!getValue(model, `/spec/admin/databases/Neo4j/mode/toggle`)) {
      let defMode = getDefault('databases/Neo4j/mode') || ''
      if (defMode === '') {
        const arr = getValue(model, '/spec/databases/Neo4j/mode/available') || []
