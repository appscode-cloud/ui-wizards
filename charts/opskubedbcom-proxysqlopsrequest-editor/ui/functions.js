...
async function getDbDetails({
  ...
  if (namespace && name) {
    ...
    resp.data = {
      ...
      spec: {
        authSecret: { name: "proxy-demo-auth" },
        autoOps: {},
        backend: { name: "mysql-demo-08-12-12-31" },
        healthChecker: {
          failureThreshold: 1,
          periodSeconds: 10,
          timeoutSeconds: 10,
        ...
        monitor: {
          ...
          prometheus: {
            exporter: { port: 56790, resources: {} },
            serviceMonitor: { interval: "30s" },
          },
        },
        initConfig: {
          mysqlVariables: {
            ping_timeout_server: 215,
            connect_timeout_server: 10234,
          },
          adminVariables: {
            refresh_interval: 2019,
            cluster_check_interval_ms: 203,
          },
          mysqlUsers: [
            {
              username: "shakib",
