...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    const name = storeGet('/route/params/name') || getValue(model, '/spec/proxyRef/name')

    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/v1alpha2/namespaces/${namespace}/proxysqls/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      const presetVersions = presets.admin?.databases?.ProxySQL?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
  ...
  // MySQL Users
  function onMySQLUserReqTypeChange() {
