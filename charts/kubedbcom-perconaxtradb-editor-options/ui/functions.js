...
export const useFunc = (model) => {
  ...
  async function initBundle() {
    ...
    if (!getValue(model, `/spec/admin/databases/PerconaXtraDB/mode/toggle`)) {
      let defMode = getDefault('databases/PerconaXtraDB/mode') || ''
      if (defMode === '') {
        const arr = getValue(model, '/spec/databases/PerconaXtraDB/mode/available') || []
  ...
  async function getPerconaXtraDBVersions(group, version, resource) {
    const owner = storeGet('/route/params/user')
    const cluster = storeGet('/route/params/cluster')
