...
export const useFunc = (model) => {
  ...
  function onScheduleBackupChange() {
    ...
    if (scheduleBackup === 'no') {
      ...
      // delete annotation from kubedbComHazelcast annotation
      deleteKubeDbComHazelcastAnnotation(getValue, model, commit)
    } else {
  ...
  function getBackupConfigsAndAnnotations(getValue, model) {
    ...
    const kubedbComHazelcastAnnotations =
      getValue(model, '/resources/kubedbComHazelcast/metadata/annotations') || {}

    const isBluePrint = Object.keys(kubedbComHazelcastAnnotations).some(
      (k) =>
  ...
  function deleteKubeDbComHazelcastAnnotation(getValue, model, commit) {
    const annotations = getValue(model, '/resources/kubedbComHazelcast/metadata/annotations') || {}
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComHazelcast/metadata/annotations',
  ...
  async function initBackupData() {
    ...
    dbResource = getValue(model, '/resources/kubedbComHazelcast')
  ...
  function onBackupTypeChange() {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComHazelcast',
  ...
  function onArchiverChange() {
    ...
    const path = 'resources/kubedbComHazelcast/spec/archiver'
    if (archiverSwitch) {
      commit('wizard/model$update', {
        path: path,
  ...
  function addLabelAnnotation(type) {
    ...
    commit('wizard/model$update', {
      path: `/resources/kubedbComHazelcast/metadata/${type}`,
      value: obj,
      force: true,
    })
  }
  ...
  function isConsole() {
    ...
    if (isKube) {
      ...
      commit('wizard/model$update', {
        path: '/resources/autoscalingKubedbComHazelcastAutoscaler/spec/databaseRef/name',
        value: dbName,
        force: true,
      })
      ...
      commit('wizard/model$update', {
        path: '/resources/autoscalingKubedbComHazelcastAutoscaler/metadata/name',
        value: modifiedName,
        force: true,
      })
      const namespace = storeGet('/route/query/namespace') || ''
      if (namespace) {
        commit('wizard/model$update', {
          path: '/resources/autoscalingKubedbComHazelcastAutoscaler/metadata/namespace',
          value: namespace,
          force: true,
        })
      }
  ...
  function onNamespaceChange() {
    ...
    const agent = getValue(model, '/resources/kubedbComHazelcast/spec/monitor/agent')
  ...
  async function getHazelcastDbs() {
    // watchDependency('model#/resources/autoscalingKubedbComHazelcastAutoscaler/metadata/namespace')
    ...
    const owner = storeGet('/route/params/user')
    const cluster = storeGet('/route/params/cluster')
    const storageEngine = getValue(model, '/resources/kubedbComHazelcast/spec/storageEngine')
    showStoragememory = storageEngine === 'inMemory'
    const resp = await axios.get(
      `/clusters/${owner}/${cluster}/proxy/kubedb.com/v1alpha2/namespaces/${namespace}/hazelcasts`,
      {
        params: { filter: { items: { metadata: { name: null } } } },
      },
    )
  ...
  function onTriggerChange(type) {
    ...
    const commitPath = `/resources/autoscalingKubedbComHazelcastAutoscaler/spec/${type}/trigger`
  ...
  function onMachineChange(type) {
    const annoPath = '/resources/autoscalingKubedbComHazelcastAutoscaler/metadata/annotations'
    ...
    const maxMachineAllocatable = maxMachineObj
      ...
      : null
    const allowedPath = `/resources/autoscalingKubedbComHazelcastAutoscaler/spec/compute/${type}`
  ...
  function setControlledResources(type) {
    ...
    const path = `/resources/autoscalingKubedbComHazelcastAutoscaler/spec/compute/${type}/controlledResources`
    commit('wizard/model$update', {
      path: path,
      value: list,
      force: true,
  ...
  function isNodeTopologySelected() {
    // watchDependency(
    //   'model#/resources/autoscalingKubedbComHazelcastAutoscaler/spec/compute/nodeTopology/name',
    // )
    const nodeTopologyName =
      getValue(
        model,
        '/resources/autoscalingKubedbComHazelcastAutoscaler/spec/compute/nodeTopology/name',
      ) || ''
  ...
  function onEnableMonitoringChange() {
    ...
    if (configureStatus) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComHazelcast/spec/monitor',
        value: {},
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/kubedbComHazelcast/spec/monitor')
    }
  ...
  function onCustomizeExporterChange() {
    ...
    if (configureStatus) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComHazelcast/spec/monitor/prometheus/exporter',
        value: {},
        force: true,
      })
    } else {
  ...
  function onLabelChange() {
    const labels = getValue(model, '/resources/kubedbComHazelcast/spec/metadata/labels')
  ...
  async function getConfigMapKeys(index) {
    ...
    const configMapName = getValue(
      model,
      `/resources/kubedbComHazelcast/spec/monitor/prometheus/exporter/env/${index}/valueFrom/configMapKeyRef/name`,
    )
  ...
  async function getSecretKeys(index) {
    ...
    const secretName = getValue(
      model,
      `/resources/kubedbComHazelcast/spec/monitor/prometheus/exporter/env/${index}/valueFrom/secretKeyRef/name`,
    )
  ...
  function isBindingAlreadyOn() {
    ...
    const isExposeBinding = !!keys.find((str) => str === 'catalogAppscodeComHazelcastBinding')
  ...
  function addOrRemoveBinding() {
    ...
    const labels = getValue(model, '/resources/kubedbComHazelcast/metadata/labels')
    const bindingValues = {
    ...
    if (value) {
      commit('wizard/model$update', {
        path: '/resources/catalogAppscodeComHazelcastBinding',
        value: bindingValues,
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/catalogAppscodeComHazelcastBinding')
    }
  }
  ...
  function onEnvArrayChange() {
    ...
    if (filteredEnv.length)
      commit('wizard/model$update', {
        path: '/resources/kubedbComHazelcast/spec/monitor/prometheus/exporter/env',
        value: filteredEnv,
        force: true,
      })
  }
  ...
  function initMonitoring() {
    const env =
      getValue(model, '/resources/kubedbComHazelcast/spec/monitor/prometheus/exporter/env') || []
