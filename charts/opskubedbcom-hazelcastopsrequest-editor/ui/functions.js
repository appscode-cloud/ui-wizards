...
export const useFunc = (model) => {
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.Hazelcast?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const resp = await axios.get(
        `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/hazelcastversions`,
        {
          params: queryParams,
        },
      )
      ...
      const filteredHazelcastVersions = sortedVersions.filter((item) => {
      ...
      setDiscriminatorValue('/filteredVersion', filteredHazelcastVersions)

      return filteredHazelcastVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
  ...
  function isReplicasValid() {
    ...
    const newReplicas = getValue(model, '/spec/horizontalScaling/hazelcast')
