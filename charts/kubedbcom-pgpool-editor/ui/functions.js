...
export const useFunc = (model) => {
  ...
  function onMachineChange(type) {
    const annoPath = '/resources/autoscalingKubedbComPgpoolAutoscaler/metadata/annotations'
  ...
  async function getConfigMapKeys(index) {
    ...
    const configMapName = getValue(
      model,
      `/resources/kubedbComPgpool/spec/monitor/prometheus/exporter/env/${index}/valueFrom/configMapKeyRef/name`,
    )
  ...
  async function getSecretKeys(index) {
    ...
    const secretName = getValue(
      model,
      `/resources/kubedbComPgpool/spec/monitor/prometheus/exporter/env/${index}/valueFrom/secretKeyRef/name`,
    )
  ...
  function onEnvArrayChange() {
    ...
    if (filteredEnv.length)
      commit('wizard/model$update', {
        path: '/resources/kubedbComPgpool/spec/monitor/prometheus/exporter/env',
        value: filteredEnv,
        force: true,
      })
  }
  ...
  function initEnvArray() {
    const env = getValue(model, '/resources/kubedbComPgpool/spec/monitor/prometheus/exporter/env')
  ...
  function initMonitoring() {
    const env =
      getValue(model, '/resources/kubedbComPgpool/spec/monitor/prometheus/exporter/env') || []
