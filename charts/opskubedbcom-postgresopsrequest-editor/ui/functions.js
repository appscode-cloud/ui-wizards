...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/postgreses/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.Postgres?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const versionsToShow = filteredPostgresVersions.length
        ? filteredPostgresVersions
