  ...
  if (isConsole({ storeGet })) {
    try {
      ...
      const filteredResources = resources.filter(
        (item) => item.value.resource.toLowerCase() === "elasticsearch"
      );
...
async function getEngines(
  ...
  if (dbName) {
    try {
      ...
      const filteredResources = resources.filter(
        (item) =>
          item.spec?.elasticsearch?.databaseRef?.name === dbName &&
          item.spec?.elasticsearch?.databaseRef?.namespace === dbNamespace
      );
