...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/elasticsearches/${name}`
      ...
      try {
        ...
        if (route.params.actions === 'scale-storage') {
          storageClassName =
            ...
            resp.data?.spec?.topology?.master?.storage?.storageClassName ||
            resp.data?.spec?.topology?.data?.storage?.storageClassName ||
            resp.data?.spec?.topology?.ingest?.storage?.storageClassName ||
            resp.data?.spec?.topology?.ml?.storage?.storageClassName ||
            resp.data?.spec?.topology?.transform?.storage?.storageClassName ||
            resp.data?.spec?.topology?.dataCold?.storage?.storageClassName ||
            resp.data?.spec?.topology?.dataContent?.storage?.storageClassName ||
            resp.data?.spec?.topology?.dataFrozen?.storage?.storageClassName ||
            resp.data?.spec?.topology?.dataHot?.storage?.storageClassName ||
            resp.data?.spec?.topology?.dataWarm?.storage?.storageClassName ||
            ''
          if (storageClassName) {
  ...
  async function getDbVersions() {
    ...
    try {
      ...
      const versionsToShow = filteredDbVersions.length
        ? filteredDbVersions
  ...
  function hasResourceValue(type) {
    ...
    const containers = topology[type]?.podTemplate?.spec?.containers || []
    const kind = dbDetails?.kind?.toLowerCase()
    return !!containers.find((container) => container.name === kind)?.resources
  }
  ...
  function isReplicasValid(type) {
    ...
    const replicaTypes = ['data', 'ingest', 'master']
  ...
  function getLimits(type) {
    const dbDetails = getValue(discriminator, '/dbDetails')
    const isTopology = !!dbDetails?.spec?.topology

    const spec = isTopology ? dbDetails?.spec?.topology?.[type] : dbDetails?.spec?.podTemplate?.spec
