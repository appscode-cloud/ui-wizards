...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/milvuses/${name}`
      ...
      if (route.params.actions === 'scale-storage') {
        storageClassName =
          ...
          resp.data?.spec?.topology?.distributed?.streamingnode?.storage?.storageClassName ||
          ''
        if (storageClassName) {
  ...
  async function getDbVersions() {
    ...
    try {
      ...
      const versionsToShow = filteredMilvusVersions.length
        ? filteredMilvusVersions
  ...
  function getLimits(type) {
    const dbDetails = getValue(discriminator, '/dbDetails')
    const isTopology = !!(dbDetails?.spec?.topology?.mode !== 'Standalone')

    const spec = isTopology
      ? dbDetails?.spec?.topology?.distributed?.[type]
