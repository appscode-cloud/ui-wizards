...
export const useFunc = (model) => {
  ...
  function onScheduleBackupChange() {
    ...
    if (scheduleBackup === 'no') {
      ...
      deleteKubeDbComClickHouseDbAnnotation(getValue, model, commit)
    } else {
  ...
  function deleteKubeDbComClickHouseDbAnnotation(getValue, model, commit) {
      ...
      backends: [
        {
          ...
          retentionPolicy: {
            name: 'clickhouse-retention-policy',
            namespace: 'demo',
          },
          storageRef: {
      ...
      sessions: [
        {
          ...
          name: 'clickhouse-frequent-backup',
          repositories: [
            {
              backend: 'clickhouse-backend',
              directory: '/clickhouse-repo',
              encryptionSecret: {
                name: 'clickhouse-encryption-secret',
                namespace: 'demo',
              },
  ...
  async function getClickHouseDbs() {
    ...
    const owner = storeGet('/route/params/user')
    const cluster = storeGet('/route/params/cluster')
    const storageEngine = getValue(model, '/resources/kubedbComClickHouse/spec/storageEngine')
    showStoragememory = storageEngine === 'inMemory'
    const resp = await axios.get(
      `/clusters/${owner}/${cluster}/proxy/kubedb.com/v1alpha2/namespaces/${namespace}/clickhouses`,
      {
        params: { filter: { items: { metadata: { name: null } } } },
      },
    )
  ...
  async function getClickHouseVersions(group, version, resource) {
    const owner = storeGet('/route/params/user')
    const cluster = storeGet('/route/params/cluster')
    ...
    return filteredClickHouseVersions
  }
  ...
  function onTriggerChange(type) {
    ...
    const commitPath = `/resources/autoscalingKubedbComClickHouseAutoscaler/spec/${type}/trigger`
  ...
  async function getConfigMapKeys(index) {
    ...
    const configMapName = getValue(
      model,
      `/resources/kubedbComClickHouse/spec/monitor/prometheus/exporter/env/${index}/valueFrom/configMapKeyRef/name`,
    )
  ...
  async function getSecretKeys(index) {
    ...
    const secretName = getValue(
      model,
      `/resources/kubedbComClickHouse/spec/monitor/prometheus/exporter/env/${index}/valueFrom/secretKeyRef/name`,
    )
  ...
  function isBindingAlreadyOn() {
    ...
    const isExposeBinding = !!keys.find((str) => str === 'catalogAppscodeComClickHouseBinding')
  ...
  function addOrRemoveBinding() {
    ...
    const labels = getValue(model, '/resources/kubedbComClickHouse/metadata/labels')
    const bindingValues = {
    ...
    if (value) {
      commit('wizard/model$update', {
        path: '/resources/catalogAppscodeComClickHouseBinding',
        value: bindingValues,
        force: true,
      })
    } else {
      commit('wizard/model$delete', '/resources/catalogAppscodeComClickHouseBinding')
    }
  }
  ...
  function onEnvArrayChange() {
    ...
    if (filteredEnv.length)
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/monitor/prometheus/exporter/env',
        value: filteredEnv,
        force: true,
      })
  }
  ...
  function initMonitoring() {
    const env =
      getValue(model, '/resources/kubedbComClickHouse/spec/monitor/prometheus/exporter/env') || []
