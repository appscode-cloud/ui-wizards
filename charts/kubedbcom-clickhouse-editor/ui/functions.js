...
function isNotShardModeSelected({ model, getValue, watchDependency }) {
  watchDependency('model#/resources/kubedbComClickHouse/spec')
  const hasShardTopology = getValue(model, '/resources/kubedbComClickHouse/spec/shardTopology')
...
function isShardModeSelected({ model, getValue, watchDependency, commit }) {
  ...
  if (resp) {
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/configSecret')
...
function showAuthSecretField({ model, getValue, watchDependency }) {
  watchDependency('model#/resources/kubedbComClickHouse/spec')
  const modelPathValue = getValue(model, '/resources/kubedbComClickHouse/spec')
...
async function getStorageClassNames(
  ...
  const path =
    mode === 'shard'
      ? '/resources/kubedbComClickHouse/spec/shardTopology/shard/storage/storageClassName'
      : '/resources/kubedbComClickHouse/spec/storage/storageClassName'
...
function setStorageClass({ getValue, commit, model, discriminator }) {
  const deletionPolicy = getValue(model, 'resources/kubedbComClickHouse/spec/deletionPolicy') || ''
  const suffix = '-retain'
  let storageClass = ''
  ...
  if (mode === 'Sharded') {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/shardTopology/shard/storage/storageClassName',
      value: storageClass,
      force: true,
    })
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/shardTopology/configServer/storage/storageClassName',
      value: storageClass,
      force: true,
    })
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/storage')
  } else {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/storage/storageClassName',
      value: storageClass,
      force: true,
    })
  }
...
function deleteDatabaseModePath({ discriminator, getValue, commit, model }) {
  const mode = getValue(discriminator, '/activeDatabaseMode')
  const modelSpec = getValue(model, '/resources/kubedbComClickHouse/spec')
  if (mode === 'Sharded') {
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/replicaSet')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/replicas')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/storage')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/podTemplate')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/configSecret')
    ...
    if (!modelSpec.shardTopology) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/shardTopology',
        value: {
          configServer: {
            replicas: 3,
            storage: {
    ...
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/shardTopology')
    ...
    if (!modelSpec.replicaSet) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/replicaSet',
        value: { name: '' },
        force: true,
      })
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/replicas',
        value: 3,
        force: true,
      })
    }
...
async function getIssuerRefsName({ axios, storeGet, getValue, model, watchDependency }) {
  const owner = storeGet('/route/params/user')
  const cluster = storeGet('/route/params/cluster')
  watchDependency('model#/resources/kubedbComClickHouse/spec/tls/issuerRef/apiGroup')
  watchDependency('model#/resources/kubedbComClickHouse/spec/tls/issuerRef/kind')
  watchDependency('model#/metadata/release/namespace')
  const apiGroup = getValue(model, '/resources/kubedbComClickHouse/spec/tls/issuerRef/apiGroup')
  const kind = getValue(model, '/resources/kubedbComClickHouse/spec/tls/issuerRef/kind')
  const namespace = getValue(model, '/metadata/release/namespace')
...
function setClusterAuthMode({ model, getValue }) {
  const val = getValue(model, '/resources/kubedbComClickHouse/spec/clusterAuthMode')
  return val || 'x509'
}

function setSSLMode({ model, getValue }) {
  const val = getValue(model, '/resources/kubedbComClickHouse/spec/sslMode')
...
function onTlsConfigureChange({ discriminator, getValue, commit }) {
  ...
  if (configureStatus) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/tls',
      value: { issuerRef: {}, certificates: [] },
      force: true,
    })
  } else {
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/tls')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/clusterAuthMode')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/sslMode')
  }
}
...
function onEnableMonitoringChange({ discriminator, getValue, commit }) {
  ...
  if (configureStatus) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/monitor',
      value: {},
      force: true,
    })
  } else {
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/monitor')
  }
...
function onCustomizeExporterChange({ discriminator, getValue, commit }) {
  ...
  if (configureStatus) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/monitor/prometheus/exporter',
      value: {},
      force: true,
    })
  } else {
...
function disableInitializationSection({ model, getValue, watchDependency }) {
  const initialized = getValue(model, '/resources/kubedbComClickHouse/spec/init/initialized')
  watchDependency('model#/resources/kubedbComClickHouse/spec/init/initialized')
  return !!initialized
}
...
function initPrePopulateDatabase({ getValue, model }) {
  const waitForInitialRestore = getValue(
    model,
    '/resources/kubedbComClickHouse/spec/init/waitForInitialRestore',
  )
  const stashAppscodeComRestoreSession_init = getValue(
    model,
    '/resources/stashAppscodeComRestoreSession_init',
  )
  const script = getValue(model, '/resources/kubedbComClickHouse/spec/init/script')
...
function onPrePopulateDatabaseChange({ commit, getValue, discriminator, model }) {
  ...
  if (prePopulateDatabase === 'no') {
    ...
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/init/waitForInitialRestore',
      value: false,
    })
    commit('wizard/model$delete', '/resources/stashAppscodeComRestoreSession_init')
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/init/script')
...
function onDataSourceChange({ commit, getValue, discriminator, model }) {
  ...
  if (dataSource === 'script') {
    ...
    if (!valueExists(model, getValue, '/resources/kubedbComClickHouse/spec/init/script'))
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/init/script',
        value: initScript,
      })
...
function initVolumeType({ getValue, model }) {
  const configMap = getValue(
    model,
    '/resources/kubedbComClickHouse/spec/init/script/configMap/name',
  )
  const secret = getValue(
    model,
    '/resources/kubedbComClickHouse/spec/init/script/secret/secretName',
  )
...
function onVolumeTypeChange({ commit, getValue, discriminator, model }) {
  ...
  if (sourceVolumeType === 'configMap') {
    ...
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/init/script/secret')

    if (
      !valueExists(model, getValue, '/resources/kubedbComClickHouse/spec/init/script/configMap')
    ) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/init/script/configMap',
        value: {
          name: '',
        },
      })
    ...
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/init/script/configMap')

    if (!valueExists(model, getValue, '/resources/kubedbComClickHouse/spec/init/script/secret')) {
      commit('wizard/model$update', {
        path: '/resources/kubedbComClickHouse/spec/init/script/secret',
        value: {
          secretName: '',
        },
      })
...
function getBackupConfigsAndAnnotations(getValue, model) {
  ...
  const mongoDB = getValue(model, '/resources/kubedbComClickHouse')
  ...
  const kubedbComClickHouseAnnotations =
    getValue(model, '/resources/kubedbComClickHouse/metadata/annotations') || {}

  const isBluePrint = Object.keys(kubedbComClickHouseAnnotations).some(
    (k) =>
...
function deleteKubeDbComMongDbAnnotation(getValue, model, commit) {
  const annotations = getValue(model, '/resources/kubedbComClickHouse/metadata/annotations') || {}
  ...
  commit('wizard/model$update', {
    path: '/resources/kubedbComClickHouse/metadata/annotations',
...
function onScheduleBackupChange({ commit, getValue, discriminator, model }) {
  ...
  if (scheduleBackup === 'no') {
    ...
    // delete annotation from kubedbComClickHouse annotation
...
async function initBackupData({ storeGet, axios, getValue, model, setDiscriminatorValue }) {
  ...
  dbResource = getValue(model, '/resources/kubedbComClickHouse')
...
function onBackupTypeChange({ commit, getValue, discriminator }) {
  ...
  commit('wizard/model$update', {
    path: '/resources/kubedbComClickHouse',
...
function onArchiverChange({ getValue, discriminator, commit, model, storeGet }) {
  ...
  const path = 'resources/kubedbComClickHouse/spec/archiver'
  if (archiverSwitch) {
    commit('wizard/model$update', {
      path: path,
...
function addLabelAnnotation(commit, storeGet, type) {
  ...
  commit('wizard/model$update', {
    path: `/resources/kubedbComClickHouse/metadata/${type}`,
    value: obj,
    force: true,
  })
}
...
function onLabelChange({ commit, model, getValue }) {
  const labels = getValue(model, '/resources/kubedbComClickHouse/spec/metadata/labels')
...
function onNameChange({ commit, model, getValue }) {
  ...
  if (hasSecretConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/configSecret/name',
      value: `${dbName}-config`,
      force: true,
    })
  }
  ...
  if (hasSecretShardConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/shardTopology/shard/configSecret/name',
  ...
  if (hasSecretConfigServerConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/shardTopology/configServer/configSecret/name',
  ...
  if (hasSecretMongosConfig) {
    commit('wizard/model$update', {
      path: '/resources/kubedbComClickHouse/spec/shardTopology/mongos/configSecret/name',
...
function getCreateAuthSecret({ model, getValue }) {
  const authSecret = getValue(model, '/resources/kubedbComClickHouse/spec/authSecret')
...
function onCreateAuthSecretChange({ discriminator, getValue, commit }) {
  ...
  if (createAuthSecret) {
    commit('wizard/model$delete', '/resources/kubedbComClickHouse/spec/authSecret')
...
function transferConfigSecret({ commit, model, getValue }, src, des) {
  const isShardedMode = getValue(model, '/resources/kubedbComClickHouse/spec/shardTopology')
  if (isShardedMode) {
    commit('wizard/model$update', {
      path: `/resources/kubedbComClickHouse/spec/shardTopology/${
      ...
      value: getValue(
        model,
        `/resources/kubedbComClickHouse/spec/shardTopology/${
...
function onSetCustomConfigChange({ discriminator, getValue, commit }) {
  ...
  if (value === 'no') {
    ...
    commit(
      'wizard/model$delete',
      '/resources/kubedbComClickHouse/spec/shardTopology/shard/configSecret',
    )
    commit(
      'wizard/model$delete',
      '/resources/kubedbComClickHouse/spec/shardTopology/configServer/configSecret',
    )
    commit(
      'wizard/model$delete',
      '/resources/kubedbComClickHouse/spec/shardTopology/mongos/configSecret',
    )
    commit('wizard/model$delete', '/resources/secret_config')
    commit('wizard/model$delete', '/resources/secret_shard_config')
    commit('wizard/model$delete', '/resources/secret_configserver_config')
...
function clearSpecModel({ commit }, dbtype) {
  if (dbtype === 'standalone') {
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComClickHouseAutoscaler/spec/${autoscaleType}/cluster`,
    )
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComClickHouseAutoscaler/spec/${autoscaleType}/sentinel`,
    )
  } else if (dbtype === 'cluster') {
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComClickHouseAutoscaler/spec/${autoscaleType}/standalone`,
    )
    commit(
      'wizard/model$delete',
      `/resources/autoscalingKubedbComClickHouseAutoscaler/spec/${autoscaleType}/sentinel`,
...
function getOpsRequestUrl({ storeGet, model, getValue, mode }, reqType) {
  ...
  if (mode === 'standalone-step') return pathConstructedForKubedb
    ...
    return `${domain}/console/${owner}/kubernetes/${cluster}/ops.kubedb.com/v1alpha1/clickhouseopsrequests/create?name=${dbname}&namespace=${namespace}&group=${group}&version=${version}&resource=${resource}&kind=${kind}&page=operations&requestType=VerticalScaling`
}
