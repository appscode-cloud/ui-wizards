...
export const useFunc = (model) => {
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.Weaviate?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const resp = await axios.get(
        `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/weaviateversions`,
        {
          params: queryParams,
        },
      )
      ...
      const filteredWeaviateVersions = sortedVersions.filter((item) => {
      ...
      setDiscriminatorValue('/filteredVersion', filteredWeaviateVersions)

      return filteredWeaviateVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
  ...
  function isVerticalScaleTopologyRequired() {
    ...
    const path = `/spec/verticalScaling/weaviate/topology`
