...
const configSecretKeys = ['config.yaml']
...
export const useFunc = (model) => {
  ...
  async function getDbDetails() {
    ...
    if (namespace && name) {
      const url = `/clusters/${owner}/${cluster}/proxy/kubedb.com/${version}/namespaces/${namespace}/qdrants/${name}`
  ...
  async function getDbVersions() {
    ...
    try {
      presetVersions = presets.admin?.databases?.Qdrant?.versions?.available || []
      const queryParams = {
        filter: {
          items: {
            metadata: { name: null },
      ...
      const resp = await axios.get(
        `/clusters/${owner}/${cluster}/proxy/catalog.kubedb.com/v1alpha1/qdrantversions`,
        {
          params: queryParams,
        },
      )
      ...
      const filteredQdrantVersions = sortedVersions.filter((item) => {
      ...
      setDiscriminatorValue('/filteredVersion', filteredQdrantVersions)

      return filteredQdrantVersions.map((item) => {
        const name = (item.metadata && item.metadata.name) || ''
