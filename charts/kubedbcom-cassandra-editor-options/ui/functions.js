...
const modeDetails = {
  Standalone: {
    description: 'Single node Cassandra without high availability.',
    text: 'Standalone',
  },
  Topology: {
...
export const useFunc = (model) => {
  ...
  function validateRackName() {
    // watchDependency('model#/spec/topology/racks')
    const regex = /^[A-Za-z][A-Za-z0-9_-]*[A-Za-z0-9]$/
    const racks = getValue(model, '/spec/topology/racks') || []
  ...
  async function initBundle() {
    ...
    if (!getValue(model, `/spec/admin/databases/Cassandra/mode/toggle`)) {
      ...
      if (defMode === '') {
        const arr = getValue(model, '/spec/databases/Cassandra/mode/available') || []
  ...
  function onBackupSwitch() {
    ...
    // Backup toggle is changed to ON/OFF. Updating backup tool to KubeStash/empty.
    commit('wizard/model$update', {
      path: '/spec/backup/tool',
