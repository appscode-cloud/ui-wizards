...
const modeDetails = {
  Standalone: {
    description: 'Single node Singlestore without high availability.',
    text: 'Standalone',
  },
  Topology: {
...
async function initBundle({ commit, model, getValue, axios, storeGet, setDiscriminatorValue }) {
  ...
  if (!getValue(model, `/spec/admin/databases/Singlestore/mode/toggle`)) {
    let defMode = getDefault({ getValue, model }, 'databases/Singlestore/mode') || ''
    if (defMode === '') {
      const arr = getValue(model, '/spec/databases/Singlestore/mode/available') || []
