...
const (
	ResourceKindHazelcastAlerts = "HazelcastAlerts"
	ResourceHazelcastAlerts     = "hazelcastalerts"
...
// +kubebuilder:resource:path=hazelcastalertss,singular=hazelcastalerts,categories={kubedb,appscode}
type HazelcastAlerts struct {
...
type HazelcastAlertsSpecForm struct {
	Alert HazelcastAlert `json:"alert"`
}

type HazelcastAlert struct {
	...
	Groups               HazelcastAlertGroups `json:"groups"`
}

type HazelcastAlertGroups struct {
	Database    HazelcastDatabaseAlert `json:"database"`
	Provisioner ProvisionerAlert       `json:"provisioner"`
	OpsManager  OpsManagerAlert        `json:"opsManager"`
}

type HazelcastDatabaseAlert struct {
	Enabled mona.SeverityFlag           `json:"enabled"`
	Rules   HazelcastDatabaseAlertRules `json:"rules"`
}
...
// HazelcastAlertsList is a list of HazelcastAlertss
type HazelcastAlertsList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	// Items is a list of HazelcastAlerts CRD objects
	Items []HazelcastAlerts `json:"items,omitempty"`
}
